...
describe("c-order-builder", () => {
  ...
  it("displays the correct number of tiles and their details", async () => {
    ...
    expect(orderItem).toEqual(expect.objectContaining(mockGetOrderItems[0]));
    ...
    expect(orderTotalDivEl.textContent).toBe(`Total Items: ${expectedItems}`);
  });
  ...
  it("updates the component when an order has been updated", async () => {
    ...
    orderItemTileEl = element.shadowRoot.querySelectorAll("c-order-item-tile");
